...
object FixedWidth:
  ...
  private def readHeaderlineAsFixedWidth(path: String, resourceName: String, optsExpr: Expr[FixedWidthOpts])(using Quotes): Expr[Any] =
    ...
    try
      ...
      // Infer column widths from all lines for better detection
      val widthInferenceLines = headerOpts match
        case HeaderOptions.FromRows(merge, dropFirst) =>
          // Use all lines including headers for width inference
          lines.drop(dropFirst)
        case HeaderOptions.Manual(_*) =>
          // Use all data lines for width inference
          lines
        case HeaderOptions.AutoGenerated =>
          // Use all data lines for width inference
          lines

      // Infer column widths once from all lines using the smarter heuristic
      val columnWidths = FixedWidthParser.inferColumnWidthsFromMultipleLines(widthInferenceLines, paddingChar)
