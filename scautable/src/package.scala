...
object table:
  ...
  enum TypeInferrer {
    ...
    /** 
     * Infers column types by inspecting only the first row of data.
     * Fast, but may be inaccurate if the first row is not representative.
     */
    case FirstRow

    /**
     * Infers column types by inspecting the first `n` rows.
     * @param n Number of rows to inspect for type inference.
     * @param preferIntToBoolean If true, prefers `Int` over `Boolean` when both are possible.
     */
    case FirstN(n: Int, preferIntToBoolean: Boolean = true)

    /**
     * Infers column types by inspecting all rows in the CSV.
     * Most accurate, but may be slower for large files.
     */
    case FromAllRows

    /**
     * Disables type inference; all columns are read as `String`.
     * Use for maximum safety or when schema is unknown.
     */
    case StringType

    /**
     * Uses a user-supplied tuple type `T` for decoding.
     * Allows for custom or manual type specification, including custom decoders.
     * @tparam T The tuple type to use for decoding each row.
     */
    case FromTuple[T]()
  }
