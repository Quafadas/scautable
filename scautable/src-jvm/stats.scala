...
object Stats:
  ...
  extension [K <: Tuple, V <: Tuple](nt: Iterator[NamedTuple[K, V]])
    ...
    /**
      ...
      *
      * This method processes an `Iterator` of `NamedTuple`s and calculates descriptive statistics
      * for each column focusing on categorical/text data patterns. All column types are supported,
      * with values converted to their string representation for analysis. The method is particularly
      * useful for understanding the distribution and variety of non-numeric data.
      *
      * The method handles missing values gracefully:
      * - `None` values in `Option` types are excluded from calculations
      ...
      *       consider converting to a collection first using the `Iterable` version.
      *
      * @return A list of named tuples, one per column, containing:
      *         - `name`: Column name (String)
      *         - `uniqueEntries`: Count of unique non-missing values in the column
      *         - `mostFrequent`: The most frequently occurring value (as Option[String]), 
      *           or None if all values are missing
      *         - `frequency`: Number of times the most frequent value appears
      *         - `sample`: A comma-separated sample of up to 5 unique values, 
      *           randomly selected and truncated to 75 characters with "..." if needed
      *
      * @example {{{
      * val data = Iterator(
      ...
      * }}}
      *
      * @see [[nonNumericSummary]] for the `Iterable` version that doesn't consume the collection
      */
    inline def nonNumericSummary =
      if !nt.hasNext then
        ...
        // Use the first row to initialize the stats structure
        val firstRow = nt.next()
        val zeroValue = firstRow.map[NonNumericStatsContext] {
          [T] => (value: T) =>
        ...
        headers.zip(asList)
          .map { case (name, stats) =>
            ...
            val (mostFrequentStr, frequency) = if stats.counts.nonEmpty then
              stats.counts.maxBy(_._2)
            else
              ("", 0)
  ...
  extension [K <: Tuple, V <: Tuple](nt: Iterable[NamedTuple[K, V]])
    ...
    private inline def zeroStatsValue = nt.head.map[StatsContext] {
      ...
      * - Columns with all missing values will show appropriate default values
      *
      * @note Unlike the `Iterator` version, this method does not consume 
      *       the collection, allowing for multiple analyses on the same dataset.
      *
      * @return A list of named tuples, one per column, containing:
      *         - `name`: Column name (String)
      *         - `uniqueEntries`: Count of unique non-missing values in the column
      ...
      * }}}
      *
      * @see [[numericSummary]] for numeric column analysis
      */
    inline def nonNumericSummary =
      val zeroValue = nt.head.map[NonNumericStatsContext] {
        [T] => (value: T) =>
    ...
    /**
      ...
      *
      * This method processes an `Iterable` of `NamedTuple`s and calculates both numeric and 
      * categorical statistics for all columns in the dataset. It automatically separates columns
      * into numeric (Int, Long, Double, or Option-wrapped versions) and non-numeric types,
      * providing comprehensive analysis in a single call.
      *
      * @note Unlike the `Iterator` version, this method does not consume the collection,
      *       allowing for multiple analyses on the same dataset.
      *
      ...
      * }}}
      *
      * @see [[numericSummary]] for numeric-only analysis
      * @see [[nonNumericSummary]] for categorical-only analysis
      * @see [[summary]] for the `Iterator` version
      */
    inline def summary =
