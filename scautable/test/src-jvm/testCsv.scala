...
class CSVSuite extends munit.FunSuite:
  ...
  test("delimiter config") {
    ...
    val csv1: CsvIterator[("col1", "col2", "col3"), (Int, Int, Int)] = CSV.resource("simple.csv", CsvOpts(HeaderOptions.Default, TypeInferrer.FromAllRows))

    val csv2: CsvIterator[("col1", "col2", "col3"), (Int, Int, Int)] = CSV.resource("simple_semicolon.csv", CsvOpts(typeInferrer = TypeInferrer.FromAllRows, delimiter = ';'))

    val csv3: CsvIterator[("col_0", "col_1", "col_2"), (String, String, String)] =
      CSV.resource("simple_semicolon.csv", CsvOpts(headerOptions = HeaderOptions.Auto, delimiter = ';'))

    val csv4: CsvIterator[("col1", "col2", "col3"), (Int, Int, Int)] = CSV.resource("simple_semicolon.csv", CsvOpts(delimiter = ';'))
...
|-|----|----|----|""".trim()
  ...
  test("CSV.resource with AutoGenerated headers parses data_without_headers.csv correctly") {
    val csv = CSV.resource("data_without_headers.csv", CsvOpts(HeaderOptions.Auto, TypeInferrer.StringType))
