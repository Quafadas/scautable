...
class ExcelWorkbookCacheTest extends FunSuite:
  ...
  val testExcelFile = {
    ...
    if (resourceUrl == null) throw new RuntimeException("Test resource SimpleTable.xlsx not found")
  ...
  test("ExcelWorkbookCache should clear all cached workbooks") {
    ...
    val testExcelFile2 = {
      ...
      if (resourceUrl == null) throw new RuntimeException("Test resource Numbers.xlsx not found")
      new java.io.File(resourceUrl.toURI).getAbsolutePath
    }
