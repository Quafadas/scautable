...
/** A lazily-evaluated iterator over a JDBC [[java.sql.ResultSet]] that surfaces rows as `NamedTuple[K, V]`.
  *
  * ===Single-use semantics===
  * Like [[io.github.quafadas.scautable.CsvIterator]], this iterator can only be traversed once. The underlying [[java.sql.ResultSet]], [[java.sql.Statement]], and
  * [[java.sql.Connection]] are **closed** when the iterator is exhausted or when [[close]] is called explicitly.
  *
  * The JDBC connection is opened **lazily** — only when the iterator is first used (first call to `hasNext` or `next()`). This means that constructing a `DbIterator` has no side
  * effects; errors from the connection or query appear on first use.
  *
  * ===Performance note===
  * Each call to `next()` allocates a `NamedTuple` wrapper around the decoded row tuple. This is the same cost as any `Iterator[NamedTuple]` in scautable and is acceptable for
  * typical analytical workloads. For very large result sets, materialise early with `.toSeq` and then use standard Scala collection operations rather than streaming row-by-row
  * from the database.
  *
...
class DbIterator[K <: Tuple, V <: Tuple] @publicInBinary private[db] (
  ...
  /** Explicitly close the underlying JDBC resources.
    *
    * Safe to call multiple times; subsequent calls are no-ops. If the iterator was never used, this is a no-op (no connection was opened).
    */
