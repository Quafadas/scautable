...
/** Entry point for compile-time database schema inference.
  ...
  *
  * The macro reads `SCAUTABLE_DB_URL`, `SCAUTABLE_DB_USER`, `SCAUTABLE_DB_PASSWORD` from the compiler's JVM system properties (`-DSCAUTABLE_DB_URL=...`) or, failing that, its
  * environment variables. If the live DB is unavailable it falls back to a JSON snapshot file (path controlled by `SCAUTABLE_DB_SNAPSHOT` or defaulting to
  * `scautable-db-schema.json`).
  *
  * ==Runtime connection==
  *
  * Generated code consults the same system properties / env vars at runtime — credentials are never baked into source.
  */
object DB:
