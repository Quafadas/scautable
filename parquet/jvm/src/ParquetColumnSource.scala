...
/** A single row group, materialised column by column.
  ...
  */
private[scautable] final case class RowGroupColumns(columns: Array[Array[Any]], rowCount: Int)
...
/** Reads a parquet file the way parquet is laid out on disk: one row group at a time, and within a row group one column chunk at a time.
  ...
  */
private[scautable] final class ParquetColumnSource(source: ParquetSource) extends AutoCloseable:
...
private[scautable] object ParquetColumnSource:
